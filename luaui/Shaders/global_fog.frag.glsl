...
float GetCameraFade()
{
	...
	float t = smoothstep(cameraFadeStart, cameraFadeEnd, cameraHeightAboveGround);
	return pow(max(0.0, t), max(0.01, cameraFadePower));
}
...
#line 33700
		...
		densityModulation = 1.0;
			...
			uint heightShadowSteps = uint(ScaledSampleCount(float(HEIGHTSHADOWSTEPS), shadowResolutionScale));
			...
			heightShadowStartPos += heightShadowEndStep * (thisQuadOffset + blueNoiseSample.x * 0.25);
			...
			uint heightNoiseSteps = uint(ScaledSampleCount(float(HEIGHTNOISESTEPS), heightFogResolutionScale));
			...
			vec3 heightFogRayStart = (quadGatherSum3D(rayStart) * 0.33) +
				(thisQuadOffset + blueNoiseSample.x * (float(heightNoiseSteps) / 64.0)) * heightFogRayStep;
...
if (cloudDensity > 0.001 && cloudGlobalColor.a > 0.001) {
	...
	if (cloudEdgeDistanceWeight > 0.0 && cloudRayLength > 0.01) {
		uint cloudSteps = uint(ScaledSampleCount(float(CLOUDSTEPS), cloudResolutionScale));
		vec3 cloudRayStep = (cloudRayEnd - cloudRayStart) / float(cloudSteps);
		float stepLength = cloudRayLength / float(cloudSteps);

		vec3 quadStep = cloudRayStep * (thisQuadOffset + (float(cloudSteps) / 128.0) * blueNoiseSample.y);
	...
	// ----------------- BEGIN CLOUD SHADOWS -------------------------------
		...
		if (shadowMapdepthRaw < 0.9998 ){ // No cloud shadows on distant areas
				...
				uint cloudShadowSteps = uint(ScaledSampleCount(float(CLOUDSHADOWS), shadowResolutionScale));
				float stepLength = length(cloudShadowRayStartPos-cloudShadowRayEndStep)/float(cloudShadowSteps); 
